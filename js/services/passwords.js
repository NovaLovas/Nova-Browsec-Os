/* passwords.js — менеджер паролей NovaOS. */
...
function unlockScreen(root, win, onUnlocked) {
  ...
  root.querySelector('.pw-forget').onclick = async () => {
    const ok = await confirm({
      ...
      message: 'Все пароли будут потеряны навсегда.',
    });
    if (!ok) return;
    await vault.wipe();
...
function mainScreen(root, win) {
  ...
  root.innerHTML = `
    <div class="app-row">
      ...
      <div class="pane"><div class="pw-detail"></div></div>
    </div>`;
  ...
  function renderDetail() {
    ...
    if (!it) {
      ...
      const b = detailEl.querySelector('[data-a="new"]');
      if (b) b.onclick = createNew;
      return;
    }
    ...
    detailEl.querySelectorAll('[data-reveal]').forEach((b) => {
      b.onclick = () => {
        const target = detailEl.querySelector(`[data-value="${CSS.escape(b.dataset.reveal)}"]`);
        if (!target) return;
...
export default function register() {
  registry.register({
    ...
    keywords: ['пароли','vault','менеджер','шифрование','password'],
    mount(root, win) {
      (async () => {
